...
import { Goal, Transaction, Category, TransactionType, PaymentMethod } from '../types';
import { Target, Plus, Trophy, X, Trash2, Sparkles, BrainCircuit, CheckCircle2, Circle, ArrowRight, Calendar, ShieldCheck } from 'lucide-react-native';
import { GoogleGenerativeAI } from "@google/generative-ai";
import { generateUUID } from '../utils/helpers'; // NOVO IMPORT
...
const GoalsManager: React.FC<Props> = ({ goals, transactions, categories, onUpdate, onAddTransaction }) => {
  ...
  // --- LÓGICA DE VERIFICAÇÃO RESERVA ---
  const handleOpenCreateModal = () => {
    const hasEmergency = goals.some(g => g.name.trim().toLowerCase() === 'reserva de emergência');

    if (!hasEmergency) {
      Alert.alert(
        "Sugestão Inteligente",
        "Vimos que você ainda não tem uma 'Reserva de Emergência'. Deseja criar uma agora?",
        [
          { 
            text: "Não, criar outra", 
            onPress: () => {
              resetForm();
  ...
  const processAnalysis = async () => {
    ...
    try {
      ...
      transactions.filter(t => t.type === TransactionType.EXPENSE).forEach(t => {
        if (mandatoryIds.has(t.id)) mandatoryTotal += t.amount;
      ...
      const prompt = `
        ...
        META: ${selectedGoal.name}, Alvo: R$ ${selectedGoal.targetAmount}, Atual: R$ ${selectedGoal.currentAmount}.
        FINANÇAS: Renda: R$ ${income}, Fixos: R$ ${mandatoryTotal}, Variáveis: R$ ${variableTotal}, Saldo Livre: R$ ${freeBalance}.
        TAREFAS: Verificar viabilidade. Sugerir plano.
        JSON: { "suggestedDate": "DD/MM/AAAA", "suggestedAllocation": 0.00, "isAchievable": boolean, "reasoning": "Texto curto" }
      `;

      const result = await model.generateContent(prompt);
      let text = result.response.text();
      text = text.replace(/```json/g, '').replace(/```/g, '').trim();
      setAiProposal(JSON.parse(text));
    } catch (error) {
  ...
  const applyAIChanges = () => {
    ...
    const updatedGoals = goals.map(g => g.id === selectedGoal.id ? { ...g, deadline: aiProposal.suggestedDate, monthlyAllocation: aiProposal.suggestedAllocation } : g);
  ...
  const handleCreate = () => {
    ...
    const initAmount = parseFloat(initialAmount.replace(',', '.') || '0');

    const newGoal: Goal = {
      id: generateUUID(), // CORREÇÃO: UUID Válido
      name,
    ...
    // CORREÇÃO: Se tem valor inicial, cria transação no extrato
    if (initAmount > 0) {
      const today = new Date().toISOString().split('T')[0];
      // Define a categoria correta: Se for Reserva, usa a especial, senão usa Compras
      const categoryName = name.trim().toLowerCase() === 'reserva de emergência' 
        ? 'Reserva de Emergência' 
        : 'Compras';

      onAddTransaction({
        amount: initAmount,
        description: `Início Meta: ${name}`,
  ...
  const confirmDeposit = () => {
    ...
    // 2. Criar Transação Extrato
    const today = new Date().toISOString().split('T')[0];
    const categoryName = selectedGoal.name.trim().toLowerCase() === 'reserva de emergência' 
        ? 'Reserva de Emergência' 
        : 'Compras';
    ...
    Alert.alert("Sucesso", "Depósito realizado e registrado no extrato!");
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}>
        {goals.map(goal => {
          ...
          const isEmergency = goal.name.trim().toLowerCase() === 'reserva de emergência';

          return (
            <View key={goal.id} style={[styles.card, isEmergency && styles.emergencyCard]}>
              ...
              <View style={styles.cardTop}>
                <View style={styles.cardIconRow}>
                  <View style={[styles.iconBox, { backgroundColor: isCompleted ? '#ecfdf5' : '#eff6ff' }, isEmergency && { backgroundColor: '#fef3c7' }]}>
                    {isEmergency ? <ShieldCheck size={20} color="#d97706" /> : (isCompleted ? <Trophy size={20} color="#10b981" /> : <Target size={20} color="#3b82f6" />)}
                  </View>
                  <View>
                    <Text style={[styles.cardName, isEmergency && {color: '#fff'}]}>{goal.name}</Text>
                    <View style={styles.dateRow}>
                       <Calendar size={12} color={isEmergency ? "rgba(255,255,255,0.7)" : "#94a3b8"} />
                       <Text style={[styles.cardDate, isEmergency && {color: "rgba(255,255,255,0.7)"}]}>{goal.deadline || 'Sem prazo'}</Text>
                    </View>
                  </View>
                </View>
                <TouchableOpacity onPress={() => handleDelete(goal.id)}>
                   <Trash2 size={18} color={isEmergency ? "rgba(255,255,255,0.7)" : "#94a3b8"} />
                </TouchableOpacity>
              </View>

              <View style={[styles.cardValues, isEmergency && {borderBottomColor: "rgba(255,255,255,0.2)"}]}>
                 <View>
                   <Text style={[styles.missingLabel, isEmergency && {color: "rgba(255,255,255,0.8)"}]}>ALOCAÇÃO MENSAL</Text>
                   <Text style={[styles.allocationValue, isEmergency && {color: "#fff"}]}>
                     {goal.monthlyAllocation ? `R$ ${goal.monthlyAllocation.toLocaleString()}` : 'Não definida'}
                   </Text>
                 </View>
                 <View style={styles.alignEnd}>
                   <Text style={[styles.missingLabel, isEmergency && {color: "rgba(255,255,255,0.8)"}]}>FALTAM</Text>
                   <Text style={[styles.missingValue, isEmergency && {color: "#fff"}]}>
              ...
              <View style={styles.cardBottom}>
                 <View style={styles.progressRow}>
                    <Text style={[styles.progressPerc, { color: isCompleted ? '#059669' : '#2563eb' }, isEmergency && {color: "#fff"}]}>
                      {progress.toFixed(0)}%
                    </Text>
                    <Text style={[styles.targetText, isEmergency && {color: "rgba(255,255,255,0.8)"}]}>de R$ {goal.targetAmount.toLocaleString()}</Text>
                 </View>

                 {!isCompleted && (
                   <TouchableOpacity onPress={() => openDepositModal(goal)} style={[styles.depositButton, isEmergency && {backgroundColor: '#fff'}]}>
                     <Plus size={14} color={isEmergency ? "#d97706" : "white"} />
                     <Text style={[styles.depositText, isEmergency && {color: "#d97706"}]}>Depositar</Text>
                   </TouchableOpacity>
                 )}
              </View>
            </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.aiModalContent}>
             <View style={styles.aiHeader}>
               ...
               <TouchableOpacity onPress={() => setShowAIModal(false)} style={styles.closeButton}><X size={20} color="#64748b" /></TouchableOpacity>
             </View>
             {wizardStep === 1 && (
               <View style={styles.wizardStep}>
                 ...
                 <ScrollView style={styles.wizardScroll}>
                   {goals.map(g => (
                     <TouchableOpacity key={g.id} onPress={() => { setSelectedGoal(g); setWizardStep(2); }} style={styles.wizardOption}>
               ...
               <View style={styles.wizardStep}>
                 ...
                 <ScrollView style={styles.wizardScroll}>
                   {transactions.filter(t => t.type === TransactionType.EXPENSE).map(t => {
                     ...
                     return (
                       <TouchableOpacity key={t.id} onPress={() => toggleMandatory(t.id)} style={[styles.checkOption, isSelected && styles.checkOptionActive]}>
                         <View style={{flex: 1}}><Text style={[styles.checkText, isSelected && {color: '#1e293b'}]}>{t.description}</Text><Text style={styles.checkSubText}>R$ {t.amount.toFixed(2)}</Text></View>
                         {isSelected ? <CheckCircle2 size={24} color="#10b981" /> : <Circle size={24} color="#cbd5e1" />}
                       </TouchableOpacity>
                     );
                   })}
                 </ScrollView>
                 <TouchableOpacity onPress={processAnalysis} style={styles.nextButton}><Text style={styles.nextButtonText}>Analisar com IA</Text><Sparkles size={18} color="#fff" /></TouchableOpacity>
               </View>
             )}
             {wizardStep === 3 && (
               <View style={styles.wizardStep}>
                 {isAnalyzing ? <View style={styles.loadingContainer}><ActivityIndicator size="large" color="#8b5cf6" /><Text style={styles.loadingText}>Calculando viabilidade...</Text></View> : aiProposal ? (
                   <ScrollView>
                     <Text style={styles.resultTitle}>Plano Gerado</Text>
                     <Text style={styles.resultReason}>{aiProposal.reasoning}</Text>
                     <View style={styles.proposalCard}>
                       <View style={styles.proposalRow}><Text style={styles.proposalLabel}>Nova Data:</Text><Text style={styles.proposalValue}>{aiProposal.suggestedDate}</Text></View>
                       <View style={styles.proposalRow}><Text style={styles.proposalLabel}>Alocação Mensal:</Text><Text style={styles.proposalValue}>R$ {aiProposal.suggestedAllocation.toFixed(2)}</Text></View>
                     </View>
                     <TouchableOpacity onPress={applyAIChanges} style={styles.authButton}><Text style={styles.authButtonText}>Autorizar</Text></TouchableOpacity>
                   </ScrollView>
                 ) : null}
               </View>
             )}
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
          <View style={styles.modalContent}>
             <View style={styles.modalHeader}>
               ...
               <TouchableOpacity onPress={() => setShowCreateModal(false)} style={styles.closeButton}><X size={20} color="#64748b" /></TouchableOpacity>
             </View>
             <ScrollView contentContainerStyle={styles.formGap}>
                <View><Text style={styles.label}>Nome da Meta</Text><TextInput placeholder="Ex: Viagem" style={styles.input} value={name} onChangeText={setName} placeholderTextColor="#94a3b8" /></View>
                <View style={styles.row}>
                  <View style={styles.flex1}><Text style={styles.label}>Valor Alvo (R$)</Text><TextInput placeholder="0,00" keyboardType="numeric" style={styles.input} value={targetAmount} onChangeText={setTargetAmount} placeholderTextColor="#94a3b8" /></View>
                  <View style={styles.flex1}><Text style={styles.label}>Data Alvo</Text><TextInput placeholder="DD/MM/AAAA" style={styles.input} value={deadline} onChangeText={setDeadline} placeholderTextColor="#94a3b8" /></View>
                </View>
                <View><Text style={styles.label}>Alocação Mensal</Text><TextInput placeholder="Quanto guardar p/ mês?" keyboardType="numeric" style={styles.input} value={monthlyAllocation} onChangeText={setMonthlyAllocation} placeholderTextColor="#94a3b8" /></View>
                <View><Text style={styles.label}>Já guardou algo?</Text><TextInput placeholder="0,00" keyboardType="numeric" style={styles.input} value={initialAmount} onChangeText={setInitialAmount} placeholderTextColor="#94a3b8" /></View>
                <TouchableOpacity onPress={handleCreate} style={styles.saveButton}><Text style={styles.saveButtonText}>Criar Meta</Text></TouchableOpacity>
             </ScrollView>
          </View>
        </KeyboardAvoidingView>
      </Modal>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
          <View style={[styles.modalContent, { height: 'auto', paddingBottom: 32 }]}>
             <View style={styles.modalHeader}>
               ...
               <TouchableOpacity onPress={() => setShowDepositModal(false)} style={styles.closeButton}><X size={20} color="#64748b" /></TouchableOpacity>
             </View>
             ...
             <TouchableOpacity onPress={confirmDeposit} style={styles.saveButton}><Text style={styles.saveButtonText}>Confirmar</Text></TouchableOpacity>
          </View>
        </KeyboardAvoidingView>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
    ...
    emergencyCard: { backgroundColor: '#f59e0b', borderColor: '#d97706' },
